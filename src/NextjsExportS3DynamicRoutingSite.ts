...
const CACHE_CONTROL_SERVER_LONG_NO_BROWSER =
...
export class NextjsExportS3DynamicRoutingSite extends Construct {
    ...
    constructor(
        ...
        private readonly props: NextjsExportS3DynamicRoutingSiteProps,
    ) {
        super(scope, id);

        this.viewerRequestCloudfrontFunction =
            this.createCloudfrontUrlRewriterFunction();
    ...
    private createCloudfrontUrlRewriterFunction(): cloudfront.Function {
        ...
        const routesManifest = fs.readFileSync(routesManifestPath, "utf8");
        ...
        if (
            parsedRoutesManifest?.version !==
            VALID_NEXTJS_ROUTES_MANIFEST_VERSION
        ) {
            throw new Error(
        ...
        if (
            parsedRoutesManifest.dataRoutes &&
            parsedRoutesManifest.dataRoutes.length > 0
        ) {
            throw new Error(
        ...
        const dynamicRoutePages =
            this.buildDynamicRoutePages(parsedRoutesManifest);

        return new cloudfront.Function(
            this,
            `${this.id}-CloudfrontUrlRewriterFunction`,
            {
     ...
     */
    private buildDynamicRoutePages(
        parsedRoutesManifest: NextjsRoutesManifest,
    ): NestedStringObject {
        return parsedRoutesManifest.dynamicRoutes.reduce((result, route) => {
    ...
    private createCloudfrontDistribution(): cloudfront.Distribution {
        return new cloudfront.Distribution(
            this,
            `${this.id}-CloudfrontDistribution`,
            {
                defaultRootObject: "index.html",
                ...this.props.distributionProps,
                errorResponses: this.props.distributionProps
                    ?.errorResponses ?? [
                    {
                        httpStatus: 403,
    ...
    private deploySiteToS3(): s3deploy.BucketDeployment[] {
        ...
        const nextExportPath =
            this.props.nextExportPath ?? DEFAULT_NEXT_EXPORT_DIR;
        ...
        result.push(
            new s3deploy.BucketDeployment(
                this,
                `${this.id}-S3BucketDeployment-Forever`,
                {
                    sources: [
                        s3deploy.Source.asset(
                            path.join(process.cwd(), nextExportPath),
                        ),
                    ],
                    destinationBucket: this.s3Bucket,
                    include: extensionsToCacheForever,
                    cacheControl: [
                        s3deploy.CacheControl.fromString(CACHE_CONTROL_FOREVER),
                    ],
                    prune: false,
                },
            ),
        ...
        result.push(
            new s3deploy.BucketDeployment(
                this,
                `${this.id}-S3BucketDeployment-Default`,
                {
                    sources: [
                        s3deploy.Source.asset(
                            path.join(process.cwd(), nextExportPath),
                    ...
                    cacheControl: [
                        s3deploy.CacheControl.fromString(
                            CACHE_CONTROL_SERVER_LONG_NO_BROWSER,
                        ),
                    ],
                    prune: false,
